...
static bool EarlyOutTest(const cv::Mat& game_img, uint32_t location_col0, uint32_t location_col1, uint32_t location_row0, uint32_t location_row1)
{
...
std::string LocationDetector::GetLocation(const cv::Mat& game_img)
{
	...
	cv::resize(game_img(cv::Rect(bbox_col0, bbox_row0, bbox_col1 - bbox_col0, bbox_row1 - bbox_row0)), location_frame, cv::Size(int((bbox_col1 - bbox_col0) / scale_factor), int((bbox_row1 - bbox_row0) / scale_factor)));
	...
	PIX* pix = pixCreateHeader(location_frame.cols, location_frame.rows, 32);
