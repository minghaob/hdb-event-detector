...
public:
	...
	static cv::Rect BBoxConversion(uint32_t width, uint32_t height)
	{
		...
		uint32_t bbox_col0 = uint32_t(bbox_x0 * double(width) + 0.5);
		uint32_t bbox_col1 = uint32_t(bbox_x1 * double(width) + 0.5);
		uint32_t bbox_row0 = uint32_t(bbox_y0 * double(height) + 0.5);
		uint32_t bbox_row1 = uint32_t(bbox_y1 * double(height) + 0.5);
		return cv::Rect(bbox_col0, bbox_row0, bbox_col1 - bbox_col0, bbox_row1 - bbox_row0);
	}
};
